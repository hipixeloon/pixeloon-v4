...
export function usePlatformConnections(userId: string | undefined) {
  ...
  const connectYouTube = useCallback(async () => {
    ...
    try {
      ...
      if (fnError) {
        throw new Error(await getEdgeFunctionErrorMessage(fnError, 'Failed to start YouTube connection'));
      }
      if (data?.error) throw new Error(data.error);
      if (data.authUrl) window.location.href = data.authUrl;
    } catch (err) {
      setError(await getEdgeFunctionErrorMessage(err, 'Failed to start YouTube connection'));
    }
  }, [userId]);
  ...
  const handleYouTubeCallback = useCallback(async (code: string) => {
    ...
    try {
      ...
      if (fnError) {
        throw new Error(await getEdgeFunctionErrorMessage(fnError, 'Failed to complete YouTube connection'));
      }
      if (data?.error) throw new Error(data.error);
      ...
      setError(await getEdgeFunctionErrorMessage(err, 'Failed to complete YouTube connection'));
      return null;
    } finally {
      setLoading(false);
    }
