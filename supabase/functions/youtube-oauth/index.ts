  ...
  try {
    ...
    if (action === 'get-auth-url') {
      ...
      const redirectUri = (parsedBody?.redirect_uri as string) || 
                          (parsedBody?.redirectUri as string) ||
    ...
    if (action === 'exchange-token') {
      ...
      const googleClientId = await getUserApiKey(resolvedUserId, 'youtube_client_id')
      const googleClientSecret = await getUserApiKey(resolvedUserId, 'youtube_client_secret')
      ...
      if (!ytResponse.ok || ytData?.error) {
        return new Response(
          JSON.stringify({ error: ytData?.error?.message || 'Failed to fetch YouTube channels' }),
          { status: ytResponse.status || 400, headers: { ...corsHeaders, 'Content-Type': 'application/json' } }
        )
      }
